...
function normalizeCourseKey(value: unknown): string {
  ...
  useState<
    Record<string, SubscriptionPlanId>
  >({});
  ...
  const [courseControls, setCourseControls] =
    useState<
      Record<
        string,
        {
  ...
  try {
    ...
    const validPlans: SubscriptionPlanId[] = [
      "monthly",
      "six_month",
      "annual",
    ];
    ...
    // Remove after applying the selection
    sessionStorage.removeItem(
      "selectedCourseId"
    );
    ...
    console.error(
      "Unable to restore selected course plan:",
      error
    );
  }
}, []);
    ...
    async (currentUser) => {
      ...
      try {
        ...
        if (profileSnapshot.exists()) {
          ...
          if (
            trialValue &&
            typeof trialValue === "object" &&
            typeof trialValue.toDate === "function"
          ) {
            setTrialStartedAt(
              trialValue.toDate()
            );
          } else if (
            typeof trialValue === "string" ||
            typeof trialValue === "number"
          ) {
            const parsedTrialDate =
              new Date(trialValue);

            if (
              !Number.isNaN(
                parsedTrialDate.getTime()
              )
            ) {
              setTrialStartedAt(
                parsedTrialDate
...
const activeCourseIds =
  purchasesSnapshot.docs
    .filter((purchase) => {
      ...
      // New subscription records must have expiresAt
      if (!data.expiresAt) {
        return false;
      }
...
// Load course publication and enrollment controls
try {
  const response = await fetch(
    "/api/courses/publication",
    { cache: "no-store" }
  );
  ...
  if (!response.ok || !data.success) {
    throw new Error(
      data.error ||
        "Unable to load course publication status."
    );
  }
  ...
  Object.entries(data.publication || {}).forEach(
    ([courseId, control]) => {
      controls[normalizeCourseKey(courseId)] =
        control as {
          published: boolean;
          enrollmentOpen: boolean;
        };
  ...
  setCourseControls(controls);
} catch (error) {
  console.error(
    "Unable to load course publication:",
    error
  );
}
      } catch (error) {
  ...
  const visibleCourses = courses.filter((course) => {
    ...
    // A paid user keeps access to a purchased course.
    if (purchased) {
      return true;
    }

    // For new aspirants, courseControls contains only
    // courses returned by the published Firestore query.
    const controlKeys = [
      course.id,
      course.title,
      course.shortTitle,
    ...
    return controlKeys.some(
      (key) => courseControls[key]?.published === true
    );
  });
  ...
  return (
    <main className="min-h-screen overflow-x-hidden bg-[#f7f8fb]">
      ...
      <header className="border-b border-gray-200 bg-white">
        <div className="mx-auto flex min-h-[82px] w-full max-w-7xl items-center justify-between px-4 sm:px-6 lg:px-8">

          <Link
            href="/"
            className="relative flex h-[65px] w-[220px] shrink-0 items-center"
          >
            <Image
              src="/logo/logo.png"
              alt="IndiaPostExam"
          ...
          <div className="flex items-center gap-2 sm:gap-3">
            <Link
              href="/dashboard"
              className="rounded-lg border border-gray-300 px-3 py-2 text-xs font-bold text-gray-700 transition hover:border-red-500 hover:text-red-600 sm:px-4 sm:text-sm"
            >
              Dashboard
            </Link>
            ...
            <Link
              href="/profile"
              className="rounded-lg bg-red-600 px-3 py-2 text-xs font-bold text-white transition hover:bg-red-700 sm:px-4 sm:text-sm"
            >
              Profile
            </Link>
          </div>
      ...
      {/* TITLE */}
      <section className="mx-auto w-full max-w-7xl px-4 pb-5 pt-8 sm:px-6 lg:px-8">
        ...
        <div className="text-center">
          ...
          <h1 className="mt-2 text-2xl font-extrabold text-[#10244a] sm:text-4xl">
            Courses for Aspirants
          </h1>

          <p className="mx-auto mt-2 max-w-xl text-sm text-gray-600">
            Choose your promotion path and start your preparation.
          </p>
        </div>

        {/* MEMBERSHIP */}
        <div className="mx-auto mt-6 flex w-full max-w-5xl flex-col gap-3 rounded-xl border border-gray-200 bg-white px-5 py-4 shadow-sm sm:flex-row sm:items-center sm:justify-between">
          ...
          <div className="w-full">
            <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
              ...
              <div>
                ...
                <p className="mt-1 text-base font-extrabold text-[#10244a]">
                  {isPremium ? "Premium Aspirant" : "Aspirant"}
                </p>
              </div>
              ...
              {!isPremium && trialActive && (
                <div className="flex items-center gap-3 rounded-xl border border-emerald-200 bg-emerald-50 px-4 py-2">
                  ...
                  <div>
                    ...
                    <p className="font-mono text-lg font-extrabold text-emerald-800">
...
return (
              ...
              >
                ...
                {/* VISUAL */}
                <div
                  className={`relative flex h-32 items-center justify-center bg-gradient-to-br ${course.theme.background}`}
                >

                  <div
                    className={`flex h-20 w-20 items-center justify-center rounded-full ${course.theme.iconBackground} text-4xl shadow-sm`}
                  >
                    {course.icon}
                  </div>
                ...
                <div className="p-5">
                  ...
                  <h2 className="mt-1 text-xl font-extrabold leading-tight text-[#10244a]">
                    {course.title}
                  </h2>
                    ...
                    {purchased ? (
                      <div>
                        <Link
                          ...
                          className="block rounded-xl bg-[#12366f] px-4 py-3 text-center text-sm font-extrabold text-white transition hover:bg-[#0e2c5c]"
                        >
                          Access Course
                        </Link>
                        ...
                        <Link
                          href={`/courses/${course.id}`}
                          className="mt-2 block text-center text-xs font-extrabold text-red-600 hover:text-red-700"
                        >
                          Renew in Advance
                        </Link>
                      </div>
                      ...
                      <div>
                        <div className="mb-3 rounded-xl border border-emerald-200 bg-emerald-50 px-4 py-3 text-center">
                          <p className="text-xs font-extrabold uppercase tracking-wide text-emerald-700">
                            FREE 1-HOUR TRIAL ACTIVE
                          </p>
                        ...
                        <Link
                          href={`/courses/${course.id}`}
                          className="block rounded-xl bg-emerald-600 px-4 py-3 text-center text-sm font-extrabold text-white transition hover:bg-emerald-700"
                        >
                          Continue Course
                        </Link>
                      </div>
                      ...
                      <div>
                        <div className="mb-3 rounded-xl border border-red-200 bg-red-50 px-4 py-3 text-center">
                          <p className="text-xs font-extrabold uppercase tracking-wide text-red-600">
                            FREE TRIAL EXPIRED
                          </p>

                          <p className="mt-1 text-sm font-semibold text-red-700">
                            Purchase this course to continue.
                          </p>
                        </div>
                        ...
                        <p className="mt-2 text-center text-xs font-semibold text-slate-500">
                          Subscription validity is calculated securely according to your selected plan.
                        </p>
                      </div>
                    ) : (
                      <div>
                        <div className="mb-3 rounded-xl border border-blue-200 bg-blue-50 px-4 py-3 text-center">
                          <p className="text-xs font-extrabold uppercase tracking-wide text-blue-700">
                            FREE 1-HOUR TRIAL
                          </p>

                          <p className="mt-1 text-sm font-semibold text-blue-800">
                            Try this course free for 1 hour.
                          </p>
...
<div className="mt-10 pb-10 text-center">
      ...
      >
        ...
        <div className="mt-4 rounded-xl bg-green-100 p-3 text-center">
          ...
          <p className="text-sm font-extrabold text-green-800">
            OK Payment verified successfully
          </p>
  ...
  return (
    <button
      ...
      className="w-full rounded-xl bg-red-600 px-5 py-4 text-base font-extrabold text-white transition hover:bg-red-700 disabled:cursor-not-allowed disabled:opacity-60"
    >
      {loading
        ? "Opening Payment..."
        : `Buy Course - Rs. ${selectedPrice}`}
    </button>
  );
}
